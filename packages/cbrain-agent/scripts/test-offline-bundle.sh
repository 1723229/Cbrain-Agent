...
if [ -n "${CBRAIN_CONFIG_FILE:-}" ]; then
  CBRAIN_API_KEY=$(node -e 'const c=require(process.argv[1]);process.stdout.write(c.apiKey||"")' "$CBRAIN_CONFIG_FILE")
...
const config=JSON.parse(readFileSync(process.env.HOME+"/.cbrain-agent/config.json","utf8"));
