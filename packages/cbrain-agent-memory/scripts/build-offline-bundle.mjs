...
const distributions = [
  { client: "codex", slug: "cbrain-codex-plugin-offline", plugin: "codex-agent-memory", marketplace: ".agents/plugins/marketplace.json", agent: "Codex" },
  { client: "claude-code", slug: "cbrain-claude-code-plugin-offline", plugin: "claude-code-agent-memory", marketplace: ".claude-plugin/marketplace.json", agent: "Claude Code" },
];
...
for (const distribution of distributions) {
  ...
  await cp(resolve(repositoryRoot, `plugins/${distribution.plugin}`), resolve(outputRoot, `plugins/${distribution.plugin}`), { recursive: true, filter: productionAsset });
...
安装器会隐藏输入 API Key，验证身份并写入 \`~/.hiper-agent-memory/config.json\`。自动化安装可通过环境变量 \`CBRAIN_API_KEY\` 传入。安装完成后重启 ${agent}。升级时解压新版 ZIP 并重新执行安装脚本。
`;
}
