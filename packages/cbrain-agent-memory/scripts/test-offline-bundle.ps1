...
$sourceConfig = Join-Path $env:USERPROFILE ".hiper-agent-memory/config.json"
...
if ((-not $connection.apiKey -and -not $connection.token) -or -not $connection.gatewayUrl) {
  throw "Existing Cbrain config does not contain a credential and gatewayUrl."
}
$verificationKey = if ($connection.apiKey) { $connection.apiKey } else { $connection.token }
...
try {
  ...
  try {
    ...
    $saved = Get-Content -Raw -LiteralPath (Join-Path $testHome ".hiper-agent-memory/config.json") | ConvertFrom-Json
    ...
    $checks = [ordered]@{
      ...
      codex_plugin_installed = [bool]($codexPlugins.installed | Where-Object pluginId -eq "codex-agent-memory@cbrain-offline")
      claude_marketplace_present = [bool]($claudeMarkets | Where-Object name -eq "cbrain-offline")
      claude_plugin_installed = [bool]($claudePlugins | Where-Object id -eq "claude-code-agent-memory@cbrain-offline")
      config_gateway_matches = $saved.gatewayUrl -eq $connection.gatewayUrl
      config_has_api_key = [bool]$saved.apiKey
      stable_codex_bundle = Test-Path -LiteralPath (Join-Path $testHome ".cbrain-agent-memory/offline/codex/bundle.json")
      stable_claude_bundle = Test-Path -LiteralPath (Join-Path $testHome ".cbrain-agent-memory/offline/claude-code/bundle.json")
    }
