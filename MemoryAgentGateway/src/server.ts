...
const worker=new CaptureWorker(store,config.core,config.captureConcurrency,config.captureMaxAttempts,config.captureTimeoutMs);
...
const wikiScope=new WikiScopeResolver(directory,async(agent,principal)=>new CoreClient(config.core,{teamId:agent.team_id,userId:principal.userId,userKey:principal.userKey,agentId:agent.agent_id,agentName:agent.name,sessionId:"wiki-mcp"}).listBoundKnowledge(config.knowledgeIds),config.knowledgeCacheTtlMs);
const app=createAgentGatewayApp({authenticate:(authorization)=>authenticator.authenticate(authorization),store,directory,serviceFactory,wikiServiceFactory:(principal)=>new WikiAccessService(()=>wikiScope.resolve(principal),knowledge),contextTtlMs:config.contextTtlMs,skillSettleMs:config.skillSettleMs});
