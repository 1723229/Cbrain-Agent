...
const authenticator=new GatewayAuthenticator({directory,cacheTtlMs:config.authCacheTtlMs,negativeCacheTtlMs:config.authNegativeCacheTtlMs,maxEntries:config.authCacheMaxEntries});
...
const drain=()=>void worker.drain().catch((error)=>console.error(`[cbrain-agent-gateway] capture worker failed: ${error instanceof Error?error.message:String(error)}`));
const timer=setInterval(()=>{store.prune();drain()},config.capturePollMs);timer.unref();drain();
serve({fetch:app.fetch,hostname:config.host,port:config.port});console.error(`[cbrain-agent-gateway] listening on ${config.host}:${config.port}`);
