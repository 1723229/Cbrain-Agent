...
export default function DefaultAgentTemplateDialog({
  ...
  async function handleSave() {
    ...
    try {
      ...
      const saved = await agentsApi.setDefaultTemplate(team.team_id, template);
      if (saved.skipped_skill_ids.length > 0) {
        tea.notify.warning(
          t('defaultAgent.publicSkillConflict', { count: saved.skipped_skill_ids.length }),
        );
      }
