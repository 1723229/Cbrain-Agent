...
export function configPath(env=process.env,home=homedir()){return env.HIPER_AGENT_MEMORY_CONFIG?.trim()||resolve(home,".hiper-agent-memory","config.json")}
export function statePath(env=process.env,home=homedir()){return env.HIPER_AGENT_MEMORY_STATE?.trim()||resolve(home,".hiper-agent-memory","state.json")}
...
async function readLegacyUserEnvironment(options){if(options.legacyEnvironment)return options.legacyEnvironment;const platform=options.platform||process.platform,release=options.release??osRelease();const command=platform==="win32"?"powershell.exe":/microsoft|wsl/i.test(release)?"powershell.exe":"";if(!command)return{};const run=options.execFile||execFileAsync;try{const script="@{gatewayUrl=[Environment]::GetEnvironmentVariable('HIPER_AGENT_GATEWAY_URL','User');mcpUrl=[Environment]::GetEnvironmentVariable('HIPER_AGENT_GATEWAY_MCP_URL','User');token=[Environment]::GetEnvironmentVariable('HIPER_AGENT_GATEWAY_TOKEN','User')} | ConvertTo-Json -Compress";const{stdout}=await run(command,["-NoProfile","-NonInteractive","-Command",script],{encoding:"utf8",windowsHide:true});const value=JSON.parse(String(stdout));return{gatewayUrl:clean(value.gatewayUrl),mcpUrl:clean(value.mcpUrl),token:clean(value.token)}}catch{return{}}}
