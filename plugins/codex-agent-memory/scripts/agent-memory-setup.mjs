...
function parseArgs(values){const result={};for(let i=0;i<values.length;i++){const key=values[i];if(key==="--gateway-url")result.gatewayUrl=values[++i];else if(key==="--api-key")result.apiKey=values[++i];else if(key==="--token")result.token=values[++i];else if(key==="--team-id")result.teamId=values[++i];else if(key==="--agent-id")result.agentId=values[++i];else if(key==="--workspace")result.workspace=values[++i];else if(key==="--name")result.name=values[++i]}return result}
