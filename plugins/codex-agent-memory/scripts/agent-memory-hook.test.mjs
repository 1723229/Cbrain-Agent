...
test("write hooks return before the Gateway response and forward the active context", async () => {
  ...
  await writeFile(config,JSON.stringify({gatewayUrl:`http://127.0.0.1:${port}`,token:"test"}),"utf8");
  const relay=await startEventRelay({gatewayUrl:`http://127.0.0.1:${port}`,token:"test"},{directory:bridgeDirectory,batchWindowMs:20});
  try{
    const env={HIPER_AGENT_MEMORY_CONFIG:config,HIPER_AGENT_MEMORY_STATE:state,HIPER_AGENT_MEMORY_BRIDGE_DIR:bridgeDirectory};
...
test("workspace resolution has a bounded timeout and a short fail-open circuit",async()=>{
  ...
  const server=createServer((_req,_res)=>{});await new Promise(resolve=>server.listen(0,"127.0.0.1",resolve));const port=server.address().port;await writeFile(config,JSON.stringify({gatewayUrl:`http://127.0.0.1:${port}`,token:"test"}),"utf8");const env={HIPER_AGENT_MEMORY_STATE:state,HIPER_WORKSPACE_KEY:"test:resolve-timeout"};
...
test("an unbound workspace asks Codex to select Team and Agent on the first prompt",async()=>{
  ...
  await new Promise(resolve=>server.listen(0,"127.0.0.1",resolve));const port=server.address().port;await writeFile(config,JSON.stringify({gatewayUrl:`http://127.0.0.1:${port}`,token:"test"}),"utf8");const env={HIPER_AGENT_MEMORY_CONFIG:config,HIPER_AGENT_MEMORY_STATE:state,HIPER_WORKSPACE_KEY:"test:unbound"};
  try{
    const started=await runHook(config,"session-start",{session_id:"s-bootstrap",cwd:directory,hook_event_name:"SessionStart"},env);const startContext=JSON.parse(started.stdout).hookSpecificOutput.additionalContext;assert.match(startContext,/hiper_workspace_binding/);assert.match(startContext,/Team A/);
...
test("a single Team and Agent pair still requires explicit confirmation",async()=>{
  ...
  const server=createServer((req,res)=>{let body="";req.on("data",chunk=>body+=chunk);req.on("end",()=>{const parsed=JSON.parse(body||"{}");requests.push({url:req.url,body:parsed});let response;if(req.url==="/v1/workspaces/resolve")response={status:"unbound",binding_request_id:"bind-one",teams:[{team_id:"team-one",name:"One"}],agents:[{agent_id:"agent-one",team_id:"team-one",name:"Codex"}]};else if(req.url==="/v1/workspaces/bind")response={status:"bound",team_id:"team-one",agent_id:"agent-one",agent_name:"Codex",context_id:"ctx-one",additionalContext:"profile"};else response={additionalContext:"recall"};res.writeHead(200,{"Content-Type":"application/json"});res.end(JSON.stringify(response))})});
  await new Promise(resolve=>server.listen(0,"127.0.0.1",resolve));const port=server.address().port;await writeFile(config,JSON.stringify({gatewayUrl:`http://127.0.0.1:${port}`,token:"test"}),"utf8");const env={HIPER_AGENT_MEMORY_CONFIG:config,HIPER_AGENT_MEMORY_STATE:state,HIPER_WORKSPACE_KEY:"test:auto"};
  try{const started=await runHook(config,"session-start",{session_id:"s-auto",cwd:directory,hook_event_name:"SessionStart"},env);assert.match(JSON.parse(started.stdout).hookSpecificOutput.additionalContext,/hiper_workspace_binding/);const prompted=await runHook(config,"user-prompt",{session_id:"s-auto",turn_id:"t1",cwd:directory,prompt:"implement it",hook_event_name:"UserPromptSubmit"},env);const context=JSON.parse(prompted.stdout).hookSpecificOutput.additionalContext;assert.match(context,/explicit confirmation/);assert.match(context,/implement it/);assert.equal(requests.some((item)=>item.url==="/v1/workspaces/bind"),false);assert.equal(requests.some((item)=>item.url==="/v1/hooks/prompt"),false)}
  finally{server.close();await rm(directory,{recursive:true,force:true})}
});
...
test("workspace_unbind clears the active local context so the next prompt asks again",async()=>{
  ...
  await new Promise(resolve=>server.listen(0,"127.0.0.1",resolve));const port=server.address().port;await writeFile(config,JSON.stringify({gatewayUrl:`http://127.0.0.1:${port}`,token:"test"}),"utf8");await writeFile(state,JSON.stringify({[key]:{contextId:"ctx-old",profile:"Agent",updatedAt:Date.now()}}),"utf8");const env={HIPER_AGENT_MEMORY_CONFIG:config,HIPER_AGENT_MEMORY_STATE:state,HIPER_WORKSPACE_KEY:"test:unbind"};
  try{await runHook(config,"tool-use",{session_id:"s-unbind",cwd:directory,tool_use_id:"call-unbind",tool_name:"mcp__hiper-agent-memory__workspace_unbind",tool_input:{workspace_key:"test:unbind"},tool_response:{removed:true}},env);assert.equal(JSON.parse(await readFile(state,"utf8"))[key],undefined);const prompted=await runHook(config,"user-prompt",{session_id:"s-unbind",cwd:directory,prompt:"continue",hook_event_name:"UserPromptSubmit"},env);assert.match(JSON.parse(prompted.stdout).hookSpecificOutput.additionalContext,/bind-again/)}finally{server.close();await rm(directory,{recursive:true,force:true})}
});
...
test("workspace_bind consumes the MCP result without a second resolve request",async()=>{
  ...
  await new Promise(resolve=>server.listen(0,"127.0.0.1",resolve));const port=server.address().port;await writeFile(config,JSON.stringify({gatewayUrl:`http://127.0.0.1:${port}`,token:"test"}),"utf8");await writeFile(state,JSON.stringify({[key]:{bindingRequestId:"bind-1",bindingExpiresAt:Date.now()+60000,bindingOptions:{teams:[],agents:[]},pendingOriginalPrompt:"implement it",updatedAt:Date.now()}}),"utf8");const env={HIPER_AGENT_MEMORY_CONFIG:config,HIPER_AGENT_MEMORY_STATE:state,HIPER_WORKSPACE_KEY:"test:bind-result"};
  try{await runHook(config,"tool-use",{session_id:"s-bind",cwd:directory,tool_use_id:"call-bind",tool_name:"mcp__hiper-agent-memory__workspace_bind",tool_input:{binding_request_id:"bind-1",team_id:"team",agent_id:"agent"},tool_response:{structuredContent:{status:"bound",context_id:"ctx-bound",team_id:"team",agent_id:"agent",agent_name:"Codex"}}},env);const saved=JSON.parse(await readFile(state,"utf8"))[key];assert.equal(saved.contextId,"ctx-bound");const prompted=await runHook(config,"user-prompt",{session_id:"s-bind",turn_id:"t1",cwd:directory,prompt:"continue",hook_event_name:"UserPromptSubmit"},env);assert.match(JSON.parse(prompted.stdout).hookSpecificOutput.additionalContext,/recalled/);assert.equal(requests.some(item=>item.url==="/v1/workspaces/resolve"),false)}finally{server.close();await rm(directory,{recursive:true,force:true})}
});

function runHook(config,mode="session-start",input={},extraEnv={}) {
  return new Promise((resolve, reject) => {
    const child = spawn(process.execPath, [fileURLToPath(new URL("./agent-memory-hook.mjs", import.meta.url)), "codex", mode], {
      env: { ...process.env, HIPER_AGENT_MEMORY_CONFIG: config, HIPER_AGENT_GATEWAY_URL: "", HIPER_AGENT_GATEWAY_MCP_URL: "", HIPER_AGENT_GATEWAY_TOKEN: "",...extraEnv },
