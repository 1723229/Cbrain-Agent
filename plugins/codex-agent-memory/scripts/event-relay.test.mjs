...
const config={gatewayUrl:"http://gateway.invalid",token:"secret"};

test("a hook datagram is batched by the matching relay without exposing the token",async()=>{
  ...
  try{
    ...
    assert.equal(await emitWriteEvent({...config,token:"other"},{type:"tool_use",body:{context_id:"wrong"}},{directory}),false);
...
test("a detached relay daemon survives its launcher and accepts later events",async()=>{
  ...
  const daemonConfig={gatewayUrl:`http://127.0.0.1:${server.address().port}`,token:"secret"};await writeFile(configPath,JSON.stringify(daemonConfig),"utf8");const env={HIPER_AGENT_MEMORY_CONFIG:configPath,HIPER_AGENT_MEMORY_BRIDGE_DIR:bridges,HIPER_AGENT_MEMORY_RELAY_IDLE_MS:"1000",HIPER_AGENT_GATEWAY_URL:"",HIPER_AGENT_GATEWAY_TOKEN:""};
...
test("a detached relay daemon recovers from a stale startup lock",async()=>{
  ...
  const daemonConfig={gatewayUrl:"http://127.0.0.1:9",token:"secret"},fingerprint=configurationFingerprint(daemonConfig),lock=join(bridges,`relay-${fingerprint}.lock`);await writeFile(configPath,JSON.stringify(daemonConfig),"utf8");await writeFile(lock,"","utf8");await utimes(lock,new Date(0),new Date(0));
  const env={HIPER_AGENT_MEMORY_CONFIG:configPath,HIPER_AGENT_MEMORY_BRIDGE_DIR:bridges,HIPER_AGENT_MEMORY_RELAY_IDLE_MS:"1000",HIPER_AGENT_GATEWAY_URL:"",HIPER_AGENT_GATEWAY_TOKEN:""};
