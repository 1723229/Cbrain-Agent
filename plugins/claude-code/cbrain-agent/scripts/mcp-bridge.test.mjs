...
test("Claude MCP bridge forwards initialize sent immediately after spawn",async()=>{
  ...
  try{
    ...
    await writeFile(config,JSON.stringify({gatewayUrl:`http://127.0.0.1:${server.address().port}`,apiKey:"test"}));
    const result=await run(config,{CBRAIN_AGENT_BRIDGE_DIR:bridges});
...
function run(config,extraEnv={}){return new Promise((resolve,reject)=>{
  const child=spawn(process.execPath,[fileURLToPath(new URL("./mcp-bridge.mjs",import.meta.url))],{env:{...process.env,CBRAIN_AGENT_CONFIG:config,CBRAIN_AGENT_GATEWAY_URL:"",CBRAIN_AGENT_API_KEY:"",...extraEnv},stdio:["pipe","pipe","pipe"]});
