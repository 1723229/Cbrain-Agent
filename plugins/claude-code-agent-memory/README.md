...
account, and writes `~/.hiper-agent-memory/config.json`. No source checkout or
...
or `解绑当前工作区`. The bundled `agent-memory` skill can also be invoked as
...
diagnostics only, run `node scripts/agent-memory-doctor.mjs` from this plugin
directory. The diagnostic output never prints the API Key.

Legacy `{ "token": ... }` configuration and `hiper-memory-local` installations
are migrated automatically; new configuration uses `{ "gatewayUrl", "apiKey" }`.
